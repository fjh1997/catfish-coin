...
<div align="center">
  ...
  <p><b>Moefish Coin（猫鱼币）</b>是一种基于 <b>DERO Homomorphic Encryption</b> 的<strong>新型实验性隐私加密货币</strong>：默认加密余额与转账、原生智能合约、CPU 可参与挖矿，并提供可双击启动的 Windows 桌面钱包。本机就能跑，也能接公网，或 fork 后自建。</p>
...
- seed 节点负责发现、同步入口，以及 **NAT 打洞信令协调**；打通后 peer 之间可直连同步，不依赖 seed 长期中转

当前版本已经修复 DERO miniblock 矿工地址哈希校验问题。v0.1.7 起历史高度兼容校验已加强；v0.1.9 起英文名改为 Moefish Coin，并更新了品牌图标与隐私币定位文案。更早的 v0.1.6 本地验证从高度 28 挖到 30，`rejected=0`，钱包余额正常增加。近期版本增加了 **多 STUN + seed 协调的 P2P NAT 打洞**，家用宽带节点在可打通场景下可建立点对点连接。
...
- 支持中文 / English 界面切换
- **公网 NAT 打洞直连**：多 STUN 候选 + seed 协调打洞，家用宽带节点可尽量建立点对点连接

## P2P NAT 打洞（Hole Punching）

家用网络通常在路由器 NAT 后面，默认只能主动连出去，别人很难直接连进来。猫鱼币在 seed 协助发现后，会尝试 **UDP 打洞直连**：

| 能力 | 说明 |
| --- | --- |
| 多 STUN 候选 | 向 Cloudflare / Google 等多个 STUN 探测公网映射；多出口 NAT 可收集多条候选地址 |
| Seed 协调 | 公网 seed 只做发现与打洞信令（交换候选、约同时发包），不替代最终数据通道 |
| 共享监听口 | 打洞探测与 KCP 会话共用同一 UDP 口，避免临时端口破坏 NAT 映射 |
| Peer-reflexive | 从入站探测包源地址学习真实可达路径（类似 Tailscale magicsock / ICE prflx） |
| 打通后可离 seed | 直连建立后，节点间同步可走点对点；seed 临时下线不影响已建立的直连 |

**打洞优势：**

- **更低延迟**：直连通常比经公网中继/seed 中转更短（实测家用节点间可达约数毫秒～十几毫秒量级）
- **减轻 seed 带宽**：区块同步与广播尽量走 peer 直连，seed 不必长期当流量中枢
- **更好的去中心化体验**：多桌面节点之间可形成网状连接，而不是永远星型挂在一台服务器上
- **NAT 友好**：不必每台机器都做端口映射或公网 IP；在可打通的 NAT 组合下自动尝试直连

说明：打洞不是 100% 成功（对称 NAT / 多出口等仍可能失败）。失败时链同步仍可通过 seed 或其它已连接 peer 进行。仅有公共 STUN、没有 seed（或其它信令通道）时，双方无法交换候选地址，仍不能单独完成打洞。
